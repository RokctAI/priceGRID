    ...
    async with async_playwright() as p:
        ...
        async def fetch_category_page(url):
            ...
            for attempt in range(3):
                try:
                    response = await cat_page.goto(
                        url, wait_until="domcontentloaded", timeout=60000
                    )
                    if response and response.status == 200:
                        break
                    logger.warning(
                        f"Attempt {attempt + 1} status: {response.status if response else 'No Response'}"
                    )
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    if not new_links:
                        logger.info(
                            f"[Cat page {cat_page_num}] No new product links -- end of catalogue."
                        )
                        break
                    ...
                    logger.info(
                        f"[Cat page {cat_page_num}] {len(new_links)} new links | {len(seen_links)} total seen"
                    )
